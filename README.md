...
- 🔴 **`font-family` に `-apple-system` / `system-ui` を書かない**（2026-09-15 に iOS で実測）。
  日本語・韓国語・中国語が**豆腐（□）**になるうえ、**後続のフォールバックまで止まる**ので、
  後ろに `"Hiragino Sans"` を並べても効かない。CJK の家族名を明示して `sans-serif` で締めること
